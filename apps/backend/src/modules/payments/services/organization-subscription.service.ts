...
export class OrganizationSubscriptionsService {
  ...
  async createCheckout(
    ...
    const priceId = this.getPriceId(tier, org.billingCycle === 'yearly');
  ...
  async updateSubscription(
    ...
    await this.stripeService.updateSubscription(subscription.stripeSubscriptionId, priceId);
  ...
  private async activateSubscription(session: any): Promise<void> {
    ...
    if (session.subscription_details?.current_period_start) {
      subscription.currentPeriodStart = new Date(session.subscription_details.current_period_start * 1000);
    }
  ...
  private getPriceId(tier: SubscriptionTier, yearly: boolean): string {
    return yearly ? `price_yearly_${tier.tier}` : `price_monthly_${tier.tier}`;
  }
